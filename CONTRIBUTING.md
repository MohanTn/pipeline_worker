...
- All configuration, including tokens, comes from `~/.config/pipeline-worker/config.json` only (`$XDG_CONFIG_HOME` honored) — never logged. No environment variables, no per-repo config file.
