...
| `PIPELINE_WORKER_AUTO_MERGE_ON_GREEN`   | `false`                      | once the MR/PR opens, ask the forge to merge it automatically as soon as CI (and any required approvals) allow — best-effort; if the forge rejects it (auto-merge not enabled for the repo, pending approvals, ...) the run continues normally and you merge manually |
